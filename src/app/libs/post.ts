...
const copyImageToPublic = (imagePath: string, postPath: string): string => {
  ...
  if (!fs.existsSync(fullImagePath)) {
    return imagePath; // 파일이 없으면 원본 경로 반환
  }
...
const getPost = (postPath: string) => {
  ...
  if (
    ...
    frontmatter.heroImage.startsWith('./')
  ) {
    frontmatter.heroImage = copyImageToPublic(frontmatter.heroImage, postPath);
  }
